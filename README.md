...
- **Rich Cursor prompt** — JD excerpts, keyword ↔ evidence table, target roles and visa notes from config, strict JSON output instructions.
...
- **Scraping** — `requests` + BeautifulSoup; URLs from `config/scrape_sources.yml`. Scheduler: `scripts/run_daily_scheduler.py`.
